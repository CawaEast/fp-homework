...
instance (Ord a) => Semigroup (Tree a) where
    (<>) Leaf a = a
    (<>) a Leaf = a
    (<>) (Node a1 b1 c1) (Node a2 b2 c2) = if a1 == a2 then Node a1 (b1 <> b2) (c1 <> c2)
                                           else if a1 < a2 then
                                                ((Node a1 b1 (c1 <> (Node a2 Leaf c2))) <> b2)
                                           else ((Node a1 (b1 <> (Node a2 b2 Leaf)) c1) <> c2)
                                           --else (Node a2 b2 c2) <> (Node a1 b1 c1)
